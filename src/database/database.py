    ...
    def search_compatibility(self, search):
        try:
            if search == None:
                ...
                sql = "select listarepuesto.id as ID, fabricante.nombre as FabricanteRepuesto, repuesto.nombre as Repuesto, modelo.nombre as Modelo, modelo.fabricacion as Año, listarepuesto.observaciones as Observaciones FROM  listarepuesto INNER JOIN repuesto ON repuesto.id = listarepuesto.repuesto_id INNER JOIN modelo ON modelo.id = listarepuesto.modelo_id INNER JOIN fabricante ON fabricante.id = repuesto.fabricante_id WHERE modelo.nombre LIKE '{0}' or modelo.fabricacion LIKE '{0}' or fabricante.nombre LIKE '{0}' or repuesto.nombre LIKE '{0}'".format(search)
                self.cursor.execute(sql)
                self.connection.commit()
